...
export class ProductService {
	...
	async uploadCnExcelPartOne(buffer: Buffer): Promise<any> {
		...
		for (let i = 0; i < rows.length; i++) {
			const [ ,hatch ] = rows[i]
			try {
				const product = await this.productRepository.save({ hatch, status: ProductStatus.IN_STORAGE })
				console.log(product)
				console.log("--------------")
			} catch(err) {
	...
	async uploadKgExcelPartOne(buffer: Buffer): Promise<any> {
		...
		for (let i = 0; i < rows.length; i++) {
			const [ ,hatch] = rows[i]
			const product = await this.productRepository.update({ hatch }, { status: ProductStatus.DELIVERED })
			console.log(product)
			console.log("-----------------")
		}
	}
